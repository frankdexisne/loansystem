    ...
    $(document).ready(function(){
        ...
        _columns = [
            ...
            {data : null ,title : 'Details', render(data){
                return 'Category: <b>'+data['category']['name']+'</b> | Terms : <b>'+data['term']['no_of_months']+(data['term']['daily_only']==0 ? ' months' : ' days')+'</b><br> Payment Mode : <b>'+data['payment_mode']['name']+'</b> | Loan Amount : <b>'+data['loan_amount_formatted']+'</b> | First Payment :'+data['first_payment']+' | Maturity date : '+data['maturity_date'];
            }},
            {data: null, title: 'Balance', render(data){
                var total_payment=0;
                $.each(data['payment'],function(){
                    total_payment+=this.amount;
                });
                
                return (parseFloat(data['loan_amount'])+(data['loan_amount']*(data['interest']/100)))-total_payment;
            }},
            {data: null, title: 'Settled',render(data){
                var total_payment=0;
                $.each(data['payment'],function(){
                    total_payment+=this.amount;
                });
            ...
            {data : null, title : 'Action', render(data,type){
                return '<div class="hidden-sm hidden-xs action-buttons">'+
                            ...
                            '<a class="blue soa" href="'+"{{url('/loans/soa')}}/"+data.id+'" title="SOA">'+
