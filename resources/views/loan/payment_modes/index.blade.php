        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form">
                    ...
                    <div class="form-group">
                        <label for="">Add days</label>
                        <input type="number" class="form-control" name="add_days" required>
                    </div>
                </form>
            </div>
...
<script>
    $(document).ready(function(){
        ...
        $element = '#datatable';
        ajaxUrl = "{{url('payment-modes-json')}}";
        ajaxType = "GET";
        ajaxData = {};
        _columns = [
            {data : 'name', title : 'Name'},
            {data : 'add_days', title : 'Add days'},
        ...
        function generateTable($element,ajaxUrl,ajaxType,ajaxData,_columns){
            ...
            $('form#form').submit(function(e){
                ...
                var form_data = {
                    ...
                    add_days : $form.find('input[name="add_days"]').val()
                };
                $.ajax({
            ...
            dataTable.on('click','a.edit',function(e){
                ...
                $td.eq(1).html('<input type="number" class="form-input" style="width:100%" value="'+$temp['add_days']+'"><span class="hidden red error-add_days"></div>');
            ...
            dataTable.on('click','a.save',function(e){
                ...
                var form_data = {
                    ...
                    add_days : $td.eq(1).find('.form-input').val() 
                };
                $.ajax({
                    url: "{{url('payment-modes')}}/"+$temp['id'],
                    type: "PUT",
                    data: form_data,
                    success: function(result){
                        // ASSIGN NEW VALUES
                        $temp['name']=$td.eq(0).find('.form-input').val();
                        $temp['add_days']=$td.eq(1).find('.form-input').val();
                        $td.eq($colIndex).find('a.edit').removeClass('edit').addClass('save');
                        $td.eq($colIndex).find('a').eq(0).find('i').removeClass('fa-pencil').addClass('fa-check');
                        $td.eq($colIndex).find('a.delete').removeClass('delete').addClass('cancel');
                        $td.eq($colIndex).find('a').eq(1).find('i').removeClass('fa-trash-o').addClass('fa-times');
